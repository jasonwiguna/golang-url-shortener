...
buildDockerImage:
	...
	CGO_ENABLED=0 gox -output="docker_releases/{{.Dir}}_{{.OS}}_{{.Arch}}/{{.Dir}}" -osarch="linux/amd64 linux/arm" -ldflags="-X github.com/jasonwiguna/golang-url-shortener/internal/util.ldFlagNodeJS=`node --version` -X github.com/jasonwiguna/golang-url-shortener/internal/util.ldFlagCommit=`git rev-parse HEAD` -X github.com/jasonwiguna/golang-url-shortener/internal/util.ldFlagYarn=`yarn --version` -X github.com/jasonwiguna/golang-url-shortener/internal/util.ldFlagCompilationTime=`TZ=UTC date +%Y-%m-%dT%H:%M:%S+0000`" ./cmd/golang-url-shortener
	docker build -t jasonwiguna/golang_url_shortener:arm -f build/Dockerfile.arm .
	docker build -t jasonwiguna/golang_url_shortener -f build/Dockerfile.amd64 .
